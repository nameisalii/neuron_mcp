...
import { CheckCircle, Slack, FileText, ExternalLink, GitBranch } from 'lucide-react'
...
export default async function IntegrationsPage({
  ...
  searchParams: { success?: string; error?: string; connected?: string }
}) {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      {searchParams.success === 'linear' || searchParams.connected === 'linear' ? (
        <div className="flex items-center gap-2 px-4 py-3 bg-green-50 border border-green-200 rounded-md">
          <CheckCircle className="w-4 h-4 text-green-600 shrink-0" />
          <p className="text-sm text-green-800">Linear connected successfully.</p>
        </div>
      ) : null}
      {searchParams.error && (
      ...
      <Card padding="md">
        <CardHeader>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg bg-[#5E6AD2] flex items-center justify-center shrink-0">
            ...
            <div className="flex items-center gap-2">
              {linear ? (
                <>
                  <SyncButton endpoint="/api/integrations/linear/sync" resultLabel="issues" />
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-green-50 text-green-700">
                    <CheckCircle className="w-3 h-3" />
                    Connected
                  </span>
        ...
        {linear && (
          <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
            <div className="bg-gray-50 rounded-md px-3 py-2">
              <p className="text-xs text-gray-400 mb-0.5">Connected</p>
              <p className="font-medium text-gray-700">{linear.createdAt.toLocaleDateString()}</p>
            </div>
            <div className="bg-gray-50 rounded-md px-3 py-2">
              <p className="text-xs text-gray-400 mb-0.5">Last synced</p>
              <p className="font-medium text-gray-700">
        ...
        {!linear && (
          <p className="text-sm text-gray-600 mt-1">
            Neuron reads Linear issues and extracts decisions, facts, and processes for semantic search.
          </p>
        )}
      </Card>
