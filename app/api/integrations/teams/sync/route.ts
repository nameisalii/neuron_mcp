...
import { syncTeams } from '@/lib/teams/sync'
...
export async function POST() {
  ...
  const integration = await prisma.integration.findUnique({
    ...
    select: { id: true, accessToken: true, channels: true, lastSyncAt: true, metadata: true },
  })
  ...
  try {
    const result = await syncTeams({
      ...
      selectedChannels: integration.channels,
      syncedBy: userId,
      syncedByName: member.displayName,
    })

    if (result.reconnectNeeded || result.adminConsentRequired || result.success) {
    ...
    console.error('[teams/sync] failed:', error instanceof Error ? error.message : 'unknown error')
    return NextResponse.json({ success: false, error: 'Microsoft Teams sync failed' }, { status: 500 })
  }
}
