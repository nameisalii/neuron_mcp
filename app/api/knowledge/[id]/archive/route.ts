...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return NextResponse.json({ item: await prisma.knowledgeItem.update({ where: { id: item.id }, data: { sourceMetadata: mergeKnowledgeMetadata(item.sourceMetadata, { knowledgeStatus: 'archived' }) as Prisma.InputJsonValue, verified: false, frozen: false } }) })
}
