...
import { generateDailyDigest, type DigestPrefs } from '@/lib/digest/generate'
  ...
  try {
    ...
    for (const member of members) {
      try {
        ...
        const content = await generateDailyDigest(member.workspaceId, member.userId, prefs)
        ...
        if (prefs.emailDigest) {
          ...
          if (user?.email) {
            await resend.emails.send({
              ...
              to: user.email,
              subject: 'Your daily Neuron digest',
              html: renderDigestEmail(content, member.displayName, 'daily'),
            })
          }
        }
        ...
        console.error(`[cron/daily-digest] member ${member.userId} failed:`, err)
      }
    }
